...
void StartTest(string one, string two, string three, string target) {
    ...
    // Measure cosine similarity or L2 norm — currently just a naive relative difference
    for (int i = 0; i < embeddingDim; ++i) {
        if (vec4[i] != 0.0f) {
            diff += fabs(testVec[i] - vec4[i]) / fabs(vec4[i]);
        }
    }

    cout << "Relative difference between 'king - man + woman' and 'queen': " << diff << endl;
}